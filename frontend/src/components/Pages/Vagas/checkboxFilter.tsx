...
const CheckboxGroup = ({ data }: CheckboxProps) => {
  const [selectedValues, setSelectedValues] = useState<{ [key: string]: { [key: string]: boolean } }>({});
  ...
  const handleChange = (category: string, element: string) => {
    setSelectedValues((prev) => {
      ...
      console.log("Estado atualizado:", newState); // Log do estado atualizado
      return newState;
    });
  };
          ...
          <div key={index} className="space-y-2">
            ...
            <div className="flex flex-col space-y-2">
              {group.elementos.map((element, idx) => (
                <label key={idx} className="inline-flex items-center space-x-2 pb-1 hover:underline">
                  <Checkbox
