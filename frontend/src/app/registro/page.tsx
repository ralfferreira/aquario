  ...
  useEffect(() => {
    const fetchCentros = async () => {
      try {
        ...
        if (!response.ok) throw new Error('Falha ao buscar centros');
  ...
  const handleRegister = async (e: FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:3001/register', {
        ...
        body: JSON.stringify({ nome, email, senha, papel, centroId, cursoId: papel === 'DISCENTE' ? cursoId : undefined }),
      });
    ...
    <div className="flex items-center justify-center min-h-screen p-2 bg-gray-50 dark:bg-transparent">
      <div className="grid w-full max-w-7xl h-auto gap-0 lg:grid-cols-2 border border-gray-200 dark:border-gray-700 dark:bg-transparent rounded-lg overflow-hidden my-12">
        <div className="relative hidden lg:flex items-center justify-center bg-sky-300 dark:bg-sky-800">
          <Image src="/logo_removebg.png" alt="Logo" width={96} height={96} className="absolute top-6 left-6 object-contain" />
        </div>

        <div className="flex items-center justify-center p-6 md:p-8 bg-white dark:bg-transparent">
          <form onSubmit={handleRegister} className="w-full max-w-md space-y-6 flex flex-col items-center">
            <div className="text-center">
              <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Crie sua conta</h1>
              <p className="text-sm text-muted-foreground dark:text-muted-foreground">Preencha os campos para se registrar</p>
            </div>
            ...
            <div className="space-y-4 w-full max-w-xs">
              ...
              <div className="space-y-2">
                <Label>Você é?</Label>
                <Select onValueChange={(value: 'DISCENTE' | 'DOCENTE') => setPapel(value)} defaultValue={papel}>
              ...
              <div className="space-y-2">
                ...
                <Select onValueChange={setCentroId} value={centroId}>
                  <SelectTrigger><SelectValue placeholder="Selecione seu centro" /></SelectTrigger>
                  <SelectContent>
                    {centros.map((centro) => (
                      <SelectItem key={centro.id} value={centro.id}>{centro.sigla} - {centro.nome}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              {papel === 'DISCENTE' && (
                <div className="space-y-2">
                  <Label>Curso</Label>
                  <Select onValueChange={setCursoId} value={cursoId} disabled={!centroId || cursos.length === 0}>
                    <SelectTrigger><SelectValue placeholder="Selecione seu curso" /></SelectTrigger>
                    <SelectContent>
                      {cursos.map((curso) => (
            ...
            <p className="text-center text-sm">
              Já tem uma conta?{' '}
              <a href="/login" className="font-semibold text-sky-500 hover:underline">Faça login</a>
            </p>
          </form>
        </div>
      </div>
