...
import { Autor } from '@/components/Shared/ProjectCard';


export default function Projetos() {
  type ProjetoComCriador = Omit<Projeto, 'autor'> & { criadorId: string, tipo: string, tags: string[], membros?: { id: string }[] };
        ...
        const projetosComAutor = await Promise.all(
          projetosData.map(async (projeto) => {
            try {
              ...
              return { ...projeto, autor: { id: '', nome: 'Desconhecido', urlFotoPerfil: null }, membros: [] };
            }
          })
        );
    ...
    <main className="px-[10%]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 py-6">
        {isLoading
          ...
          const matchesSearchTerm = projeto.titulo.toLowerCase().includes(searchTermLower) || projeto.descricao.toLowerCase().includes(searchTermLower);

          const matchesProjectType = activeButton === 'Todos' || 
                                     (activeButton === 'Pessoais' && projeto.tipo === 'PESSOAL') || 
                                     (activeButton === 'Laboratórios' && projeto.tipo === 'LABORATORIO') || 
                                     (activeButton === 'Grupos e Ligas' && (projeto.tipo === 'GRUPO' || projeto.tipo === 'LIGA'));

                    const matchesTags = tagTermLower === '' || (projeto.tags && projeto.tags.some((tag: string) => tag.toLowerCase().includes(tagTermLower)));

                    const matchesCollaborators = (() => {
            if (collaborators === '') return true;
            if (!projeto.membros) return false;
          ...
          return matchesSearchTerm && matchesProjectType && matchesTags && matchesCollaborators;
        }).map((projeto) => (
